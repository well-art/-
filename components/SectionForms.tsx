...
import { Table, ChevronRight, ArrowLeft, Archive, Layers, MousePointerClick } from 'lucide-react';
import { SharedForm } from '../types';
import { FORM_CATEGORIES } from '../constants';

interface Props {
  data: SharedForm[];
  onItemClick: (item: SharedForm) => void;
  searchTerm: string;
}

const SectionForms: React.FC<Props> = ({ data, onItemClick, searchTerm }) => {
  ...
  const selectedCategoryName = FORM_CATEGORIES.find(c => c.id === selectedCategoryId)?.name;
         ...
         <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-brand-gray flex items-center gap-2">
                共用表格 
                <span className="text-sm font-normal text-gray-400 ml-1 hidden sm:inline">Shared Forms</span>
              </h2>
              {/* 麵包屑導航 */}
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            {displayData.length === 0 ? (
               <div className="flex flex-col items-center justify-center text-center py-12 opacity-60">
                ...
                {isCategorySelected && (
                   <button onClick={() => handleCategoryClick(null)} className="mt-4 text-brand-green hover:underline">返回表格列表</button>
                )}
              </div>
            ) : (
